...
export function isLowEffortGlsl(glsl) {
  ...
  const blobMask = /smoothstep\s*\(\s*\w+\s*,\s*\w+\s*-\s*[\d.]+\s*,\s*dist\)/.test(code)
    || /smoothstep\s*\(\s*size/.test(code);
  const hasField = /fract\s*\(|hash|noise|fbm|atan\s*\(|polar|kaleidoscope|domain|ripple|voronoi|cos\s*\(\s*6\.28|mix\s*\(vec3/.test(code);
  const lineCount = code.split("\n").filter(l => l.trim() && !l.trim().startsWith("//")).length;

  return blobMask && !hasField && lineCount < 35;
}
