/** /gi flag is required: sanitizer uses .replace() and needs to strip every occurrence, not just the first.
 *  Hyphenated prefixes (`self-organiz`, `non-local`) intentionally allow trailing word chars so they match `self-organizing`, `non-locally`, etc. */
const BANNED_WORDS = "systemic|cognition|emergence|directive|morphology|friction|heuristic|anticipatory|intentional|predictive|coherence|evolutionary|distributed|intelligence|novelty|profound|serenity|unwavering|pioneering|architect|aesthetic|modeling|adjustment|progression|intentionality|manifesto|genome|emergent|self-correct";
const BANNED_PREFIXES = "self-organiz|non-local";
export const STRATEGY_BANNED_RE = new RegExp(`\\b(?:${BANNED_WORDS})\\b|\\b(?:${BANNED_PREFIXES})\\w*`, "gi");
