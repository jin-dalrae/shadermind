...
export async function consolidateMemory(db, runGeminiBatch, { fromGen, toGen }) {
...
Current strategy excerpt: ${(db.currentStrategy || "").slice(0, 1500)}`;
  ...
  const raw = await runGeminiBatch(systemPrompt, userPrompt, true, `memory consolidation ${fromGen}-${toGen}`);
